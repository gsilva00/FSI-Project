...
In more detail, the function creates a cipher object using the AES algorithm and the CTR mode, then creates an encryptor object using the cipher object, and finally encrypts the message using the encryptor object into the initialized empty byte string. The encrypted message is then returned.
...
**Answer 1:** The process starts by generating a key using the `gen()` function. Then, we can encrypt a message using the `enc()` function, passing the key, the message, and a nonce as parameters. To decrypt the message, we use the `dec()` function, passing the key, the ciphertext, and the same nonce used for encryption.
...
> Timing the brute-force attack was executed in a Dell Latitude 5490 laptop with an Intel® Core™ i5-8350U CPU @ 1.70GHz×8, 16GB RAM, running Ubuntu 20.04.5 LTS. Naturally, the time will be lower in a more powerful computer, or through the use of parallel computation. Therefore, it is just an estimate, but it serves to show that it is feasible to break the key in a reasonable amount of time.
...
- To confirm this value, we can test **256^3 == 16777216** possibilities in **490.19 seconds**. This means we can test **16777216 / 490.19 == 34225.945041719** possibilities per second (around the same value).

We can extrapolate how many possibilities we could test in 10 years:
...
> Again, it is important to note that the number of possibilities per second is not constant, and it may vary depending on the machine used. However, the value obtained is a good estimate for the purpose of this exercise.
...
- If we want to keep following a brute-force approach, we just need to test all 2^8 possibilities for each possible key, (resulting in **256^3\*256 == 256^4 == 4294967296** possibilities), which following the same logic as before, would take **4294967296 / 33573.904864822 == 127925.759999998 seconds == 2132.096 minutes == 35.534933333 hours** which is significantly longer than only having to test the possibilities for the last 3 bytes, but still feasible to break the key in a reasonable time.

- Due to the smaller size of the nonce (in AES it's always 128 bits or 16 bytes), the chances of repeating the same nonce are higher (collisions). That is, because of [how the AES Counter Mode encryption works](/images/CTF11/scheme_CTR_encryption.png), for two different messages, given the same key, the same nonce would generate the same keystream. Furthermore, naturally, the more messages we encrypt with the same key, the higher the chances of repeating the nonce (birthday paradox), whose value can only be 256 possibilities. With that information, the attacker can XOR two ciphertexts ( **C1⊕C2 == (M1⊕K)⊕(M2⊕K) == M1⊕M2**, where the same keystream (K) cancels itself ), and if the attacker knows one of the plaintexts, they can find the other one ( **M2 == (C1⊕C2)⊕M1 == M1⊕M2⊕M1**, where plaintext1 cancels itself as well). Even if a message is not fully known, the attacker can still find patterns in the plaintext, which leads him to proceed in the same way.

In conclusion, the use of a 1-byte nonce that is not sent over the network is not an effective countermeasure to strengthen the security of these cipher schemes, as it is still feasible to break the key in a reasonable time, and it increases the chances of repeating the nonce, which is a major vulnerability in the encryption process, even though it is not sent over the network and cannot therefore be intercepted.
