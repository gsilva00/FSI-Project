...
To try and confirm our suspicions, as done in the other CTFs, we tried to find vulnerabilities that affected the specified version of the NotificationX plugin in a [CVE database](https://cve.mitre.org/cgi-bin/cvekey.cgi?keyword=NotificationX). Given that 2 of the 3 vulnerabilities found ([CVE-2020-36744](https://www.cve.org/CVERecord?id=CVE-2020-36744), vulnerability relating to Cross-Site Request Forgery (CSRF) [CVE-2022-0349](https://www.cve.org/CVERecord?id=CVE-2022-0349), vulnerability relating to SQL Injection) were already patched in versions >1.8.2 and >=2.3.9, respectively, only 1 vulnerability was left. [CVE-2024-1698](https://www.cve.org/CVERecord?id=CVE-2024-1698) affected all versions <= 2.8.2 and it was a SQL Injection vulnerability, so it was a very plausible candidate for the vulnerability we were looking for.

## Finding an exploit and exploiting the vulnerability

Due to the high likelihood of this being the vulnerability we were looking for, we decided to try looking for an exploit for it. By simply searching on Google for "CVE-2024-1698 exploit", we came across [this exploit](https://github.com/kamranhasan/CVE-2024-1698-Exploit). We downloaded and analyzed it, to understand how it worked, and answer the following questions.
...
3. Finally, the code sends a POST request to find the admin password hash, character by character, using the same method as the username.
...
> This output is due to the fact that the default delay value was 0.3 seconds, which was too low for the server to respond in time, and the code interpreted that as:
>
> - the correct username length being found (1) - due to `if resp_length.elapsed.total_seconds() > delay`
> - the correct first character of the username being the NULL byte (first one checked in `for ascii_val_username in (b"\x00" + string.printable.encode())`), which marks the end of the username - due to `if resp_username.elapsed.total_seconds() > delay`
> - the first character of the password hash being the NULL byte (first one checked in `for ascii_val_password in (b"\x00" + string.printable.encode())`), which marks the end of the password (and consequently, the exploit) - due to `if resp_password.elapsed.total_seconds() > delay`
...
This was a significantly different output, the username is now the complete word `admin` and the password hash is way longer and different. We then increased the delay to 3 seconds. After many tries, we experienced lower username lengths, different letters on the username (e.g. `adein`, etc...), and other weird outputs.

The delay just increases the time frame for the server to respond, and consequently, for the exploit to consider what a correct character is. So, to account for any possible connection instability, we decided to increase the delay to 10 seconds. This makes the exploit more reliable, but also way slower, as it has to wait 10 seconds for each character of the username and password hash:
...
After 5 tries, the exploit always outputted the same username and password hash as above, so we can consider this the correct output.
